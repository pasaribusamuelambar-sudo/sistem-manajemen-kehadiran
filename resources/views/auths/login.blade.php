@extends('layouts.app')
@section('title', 'Login | PresensiHub')
@section('body-class', 'bg-gradient-custom')
...
<div class="flex flex-col md:flex-row w-full max-w-5xl bg-white shadow-2xl rounded-[40px] overflow-hidden border border-slate-100">
    ...
    <div class="w-full md:w-1/2 p-10 md:p-20 flex flex-col justify-center">
        <div class="mb-12">
            ...
            <p class="text-slate-500 font-medium mt-3">Silakan masukkan akun Anda.</p>
        </div>
        ...
        <form action="{{ route('login.post') }}" method="POST" class="space-y-6">
            ...
            <div>
                <label class="block text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3 ml-1">Masuk Sebagai</label>
                <div class="relative group">
                    <span class="absolute inset-y-0 left-0 flex items-center pl-4 text-slate-400 group-focus-within:text-indigo-600 transition-colors">
        ...
        <p class="text-center mt-10 text-sm text-slate-500 font-medium">
            Belum punya akun? 
            <a href="{{ route('register') }}" class="text-indigo-600 font-extrabold hover:underline transition-all">Daftar Akun</a>
        </p>
    </div>
</div>
@endsection